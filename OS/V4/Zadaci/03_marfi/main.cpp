...
void kasa(int broj_kupaca, duration<double, milli>& vreme_na_kasi) {
	// Implementirati...
    uniform_int_distribution<int> millis(1, 300);
    high_resolution_clock::time_point t = system_clock::now();
    for (int i = 0; i < broj_kupaca; i++) {
        duration<double> delay = milliseconds(millis(gen));
        this_thread::sleep_for(delay);
    }
    vreme_na_kasi = system_clock::now() - t;
}
...
int main() {
    ...
    // Implementirati...
    cout << vremena[0].count() << endl;
    cout << vremena[1].count() << endl;
    cout << vremena[2].count() << endl;
    int max = 0;
    for (int i = 1; i < UKUPNO_KASA; i++)
        if (vremena[i] > vremena[max])
            max = i;
    ...
    if (max == moja_kasa)
        ...
        cout << "Marfi, u ovoj konkretnoj situaciji, je na zalost omanuo." << endl;
